// src/front/components/QuickVote.jsx - ENHANCED with live event broadcasting
import React, { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import toast from 'react-hot-toast';
import authService from '../store/authService';
import GameImage from './GameImage';
import LiveMemberStatus from './LiveMemberStatus'; // Your existing component
...
const QuickVote = ({ groupId }) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (data.session) {
                    ...
                    setShowLiveStatus(true); // Show live status for existing sessions
    ...
    const startVotingSession = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                setSessionStarted(true);
                setShowLiveStatus(true); // 🚀 Show live status
                
                // 🚀 NEW: Broadcast session start event
                broadcastEvent(data.session.id, 'session_started', {
    ...
    // 🚀 ENHANCED: Vote submission with live broadcasting
    ...
    // Handle all voted callback from LiveMemberStatus
    ...
    if (submitted) {
        return (
            <div className="space-y-6">
                ...
                {/* 🚀 NEW: Show live member status even after voting */}
    ...
    // Main voting interface with live status
    return (
        <div className="space-y-6">
            {/* 🚀 NEW: Live member status panel */}
            {showLiveStatus && sessionId && (
                <LiveMemberStatus 
                    sessionId={sessionId} 
                    groupId={groupId}
                    ...
                    <>
                        ...
                        <div className="mt-6 p-4 bg-blue-500/10 border border-blue-500/30 rounded-xl text-blue-300 text-sm">
                            <div className="flex items-start space-x-2">
                                ...
                                <div>
                                    <strong>Live Voting Tips:</strong>
                                    <ul className="mt-2 space-y-1">
                                        <li>• See who has voted in real-time above! 👆</li>
                                        <li>• Select games in order of preference (1st, 2nd, 3rd choice)</li>
                                        <li>• Everyone gets notified when you vote ⚡</li>
                                        <li>• The game with the most points wins! 🏆</li>
                                        <li>• Auto-redirect when everyone has voted 🚀</li>
                                    </ul>
                                </div>
                            </div>
                        </div>
