...
export const GamingLink = ({ children, to, variant = "primary", className = "", ...props }) => {
  ...
  return (
    <Link
      ...
      to={to}
      onClick={handleClick}
      className={`inline-block text-center no-underline relative overflow-hidden px-8 py-3 rounded-xl font-semibold transition-all duration-300 transform magnetic ${getVariantClasses()} ${props.disabled ? 'opacity-50 cursor-not-allowed' : ''} ${className}`}
      {...props}
    >
...
export const GamingButton = ({ children, variant = "primary", className = "", onClick, ...props }) => {
  ...
  return (
    <button
      ref={buttonRef}
      className={`relative overflow-hidden px-8 py-3 rounded-xl font-semibold transition-all duration-300 transform magnetic ${getVariantClasses()} ${props.disabled ? 'opacity-50 cursor-not-allowed' : ''} ${className}`}
      onClick={handleClick}
      {...props}
    >
      <span className="relative z-10">{children}</span>
...
export const GamingCard = ({ children, className = "", ...props }) => {
  ...
  return (
    <div
      ref={cardRef}
      className={`gaming-card backdrop-blur-xl bg-white/10 border border-white/20 rounded-3xl p-8 shadow-2xl transition-all duration-300 preserve-3d ${className}`}
