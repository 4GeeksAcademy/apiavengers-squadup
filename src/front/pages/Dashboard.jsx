// src/front/pages/Dashboard.jsx - ENHANCED VERSION combining best of both implementations
...
export const Dashboard = () => {
    ...
    const loadDashboardData = async () => {
        ...
        try {
            ...
            if (groupsResponse.ok) {
                ...
                const errorMessage = errorData.error || 'Failed to load groups';
            ...
            const enhancedData = { 
                stats: { 
                    ...
                    favoriteGame: user?.steam_connected || user?.is_steam_connected ? 'Valorant' : 'Connect Steam to see stats',
            ...
            const errorMessage = error.message || 'Could not load dashboard data';
            setError(errorMessage);
    ...
    const fetchCommonGames = async (userGroups = groups) => {
        try {
            ...
            const allMembers = userGroups.flatMap(g => g.members?.filter(m => m.steam_connected || m.is_steam_connected) || []);
    ...
    // Enhanced group joined functionality
    ...
    const handleGroupUpdate = (action, wasDeleted, groupIdOrData) => {
        console.log('🔄 Group update received:', { action, wasDeleted, groupIdOrData });
        
        if (wasDeleted || action === 'deleted' || action === 'auto_deleted' || action === 'auto_deleted_manual') {
            // Remove the group from state completely
            setGroups(prevGroups => {
                const groupId = typeof groupIdOrData === 'object' ? groupIdOrData.id : groupIdOrData;
            ...
            fetchCommonGames();
        } else if (action === 'left' || action === 'left_with_transfer' || action === 'member_left') {
        ...
        return (
            <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-indigo-900 pt-24 px-4 pb-12 flex items-center justify-center">
                <div className="backdrop-blur-xl bg-white/10 border border-white/20 rounded-3xl p-8 text-center">
                    ...
                    <p className="text-white/70">Loading your gaming dashboard...</p>
                    <p className="text-white/50 text-sm mt-2">Setting up your squad...</p>
                </div>
            </div>
        );
    }
                ...
                <div className="max-w-7xl mx-auto">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                        ...
                        <div className="backdrop-blur-xl bg-white/10 border border-white/20 rounded-2xl p-6 text-center hover:bg-white/15 transition-all duration-300 group">
                            ...
                            <div className="mt-2 text-xs text-white/50">
                                {user.steam_connected || user.is_steam_connected ? 'Steam connected' : 'Connect Steam for more'}
                            </div>
                        </div>
                        <div className="backdrop-blur-xl bg-white/10 border border-white/20 rounded-2xl p-6 text-center hover:bg-white/15 transition-all duration-300 group">
                    ...
                    <div className="fixed bottom-6 right-6 flex flex-col space-y-3 z-50">
                        ...
                        {/* Join group via invite floating button */}
                        <button 
                            onClick={() => {
